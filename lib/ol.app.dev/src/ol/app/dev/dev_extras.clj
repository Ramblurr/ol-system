...
;; See LICENSE.juxt in project root for license info
;; sourced on 2022-01 from https://github.com/juxt/edge/blob/master/lib/edge.app.dev/src/dev_extras.clj
(ns ^{:clojure.tools.namespace.repl/load false} ol.app.dev.dev-extras
...
(defn go
  ...
  (let [res (integrant.repl/go)]
    (doseq [message (system.meta/useful-infos system-config system)]
      (println (io.aviso.ansi/yellow (format "[System] %s" message))))
    (println (str (io.aviso.ansi/yellow "[System] Now make code changes, then enter ")
